...
public class FileHelper {
    ...
    public void saveCollectionToFile (Collection<?> collection) {
        if (System.getenv().get(env) != null) {
            try (FileOutputStream fileOutputStream = new FileOutputStream(System.getenv().get(env))) {
    ...
    public HashSet<Ticket> readCollectionFromFile() throws IOException {
        ...
        if (System.getenv().get(env) != null) {
            System.out.println("a");
            try (FileInputStream fileInputStream = new FileInputStream(System.getenv().get(env))) {
                BufferedInputStream bufferedInputStream = new BufferedInputStream(fileInputStream);
                Type collectionType = new TypeToken<HashSet<Ticket>>() {
                }.getType();
